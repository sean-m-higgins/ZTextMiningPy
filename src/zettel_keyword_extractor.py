...
class ZettelPreProcessor:
    ...
    def __init__(self, zets):
        ...
        # TODO why below?
        self.stop_words = [line[:-1] for line in stopwords_file.readlines()]  # TODO remove title, note... from file
    ...
    def create_text_rank(self):
        ...
        vocab = self.create_vocab(self.lemmatized_tokens)
        token_windows = self.create_token_windows(self.lemmatized_tokens)
        ...
        for zettel in self.lemmatized_tokens:
            ...
            for word in zettel:
                ...
                cur_total_score = ((cur_tf_idf * self.score_weights[0]) + (cur_word_score * self.score_weights[1]) +
                                   (cur_keyword_score * self.score_weights[2]) + (
                                               cur_text_rank * self.score_weights[3]) +
                                   (cur_pos_score * self.score_weights[4]) + (
                                               cur_area_score * self.score_weights[5])) / 6
...
class Weights:
    ...
    def __init__(self):
        ...
        self.all_score_weights = [0.50, 0.05, 0.20, 0.05, 0.1,
                                  0.1]  # [0.40, 0.05, 0.30, 0.05, 0.1, 0.1]  #[1/6, 1/6, 1/6, 1/6, 1/6, 1/6]
        self.pos_switch = {
            'NN': 0.40,
            'NNS': 0.35,
            'NNP': 0.80,
...
if __name__ == "__main__":
    ...
    zettels = get_zettels_from_clean_directory(clean_baseball)  # TODO
